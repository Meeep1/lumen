...
- **`lumen.xcodeproj` is still entirely gitignored** — the Xcode project (targets, schemes, the `LumenTests` target, build settings) exists only on this one Mac with no backup. Flagged as an open question since Phase 2; still undecided.
